...
class FXMLViewControllerTest {
    ...
    /*
    private final MainOperationHandler mainActBtnsHandler = new MainOperationHandler(this);
    private final ExtraOperationHandler extraActBtnHandler = new ExtraOperationHandler();
    private final PercentHandler percentController = new PercentHandler(this);
    ...
    private void testFirstChain(long displayedNumber, MainOperation operation, long expResult) throws MyException {

        BigDecimal display = BigDecimal.valueOf(displayedNumber);
        display = setDisplay(mainActBtnsHandler.doOperation(operation));
    ...
    private void testSecondChain(long firstNum, long secondNum, long thirdNum, long fourthNum,
        ...
        assertEquals(addCapacity(expResult+""), mainActBtnsHandler.doEnter(new BigDecimal(display)));
        mainActBtnsHandler.resetValues();
    }
    ...
    private void testFifthChain(long firstNum, long secondNum, MainOperation mOperation, Percent percent, String result){
        ...
        setInputtedNumber(true);
        String display = percentController.doOperation(percent,BigDecimal.valueOf(firstNum),BigDecimal.valueOf(secondNum));
        display = mainActBtnsHandler.doEnter(new BigDecimal(replaceComaToDot(removeCapacity(display))));
        display = mainActBtnsHandler.doEnter(new BigDecimal(replaceComaToDot(removeCapacity(display))));
        display = mainActBtnsHandler.doEnter(new BigDecimal(replaceComaToDot(removeCapacity(display))));
    ...
    private void testSeventhChain(long first,long second,long third,MainOperation mOperation,Percent percent,String result){
        ...
        setInputtedNumber(true);
        String display = percentController.doOperation(percent,BigDecimal.valueOf(first),BigDecimal.valueOf(second));
        display = mainActBtnsHandler.doOperation(mOperation,new BigDecimal(replaceComaToDot(removeCapacity(display))));
        setInputtedNumber(true);
        display = percentController.doOperation(percent,new BigDecimal(replaceComaToDot(removeCapacity(display))),BigDecimal.valueOf(third));
        display = mainActBtnsHandler.doEnter(new BigDecimal(replaceComaToDot(removeCapacity(display))));
        assertEquals(result,mainActBtnsHandler.doEnter(new BigDecimal(replaceComaToDot(removeCapacity(display)))));
        mainActBtnsHandler.resetValues();
    }
